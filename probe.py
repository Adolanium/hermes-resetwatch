...
COMMAND_CODE_DEFAULT_BASE_URL = "https://api.commandcode.ai"
...
def _commandcode_api_key() -> Optional[str]:
    return _hermes_env_value("COMMAND_CODE_API_KEY")
...
def _commandcode_url(path: str, *, org_id: Optional[str] = None, since: Optional[str] = None) -> str:
    ...
    return f"{COMMAND_CODE_DEFAULT_BASE_URL}{path}{'?' + query if query else ''}"
...
def _commandcode_snapshot(
    ...
    if not windows and not details and plan:
        details.append("Subscription data available")
    if not windows and not details:
        return None
    return _snapshot(
        "commandcode",
...
def _fetch_commandcode_account_usage() -> Optional[dict]:
    ...
    with httpx.Client(timeout=HTTP_TIMEOUT) as client:
        account = _commandcode_account(
            _commandcode_get(client, _commandcode_url("/alpha/whoami"), headers)
        )
        if not account:
            return None
...
def _collect_cli() -> tuple[list[dict], bool]:
    ...
    fetchers.extend(
        (
            ...
            _fetch_commandcode_account_usage,
        )
    )
