...
- **Command Code:** `COMMAND_CODE_API_KEY` in Hermes env (credits, 5-hour / weekly windows, subscription, and usage)
...
- **Env keys.** If Kimi or GLM CLI login is missing, Hermes env keys fill the same cards. DeepSeek, OpenCode Go, Ollama Cloud, MiniMax, Novita, DeepInfra, AI Gateway, and Command Code use Hermes env (process env or `$HERMES_HOME/.env`).
